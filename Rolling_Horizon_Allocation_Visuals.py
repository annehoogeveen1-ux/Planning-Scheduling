...
def compute_kpis(assignment_map, remaining_capacity, providers, all_weeks, distance_log) -> dict:
    ...
    return {
        ...
        'weekly_utilization': utilization # Outputted map of raw data for graphing
    }
...
def plot_utilization(method_name: str, result: dict):
    """
    Plots the year-long weekly utilization for all providers on a single chart
    based on the respective dispatching algorithm.
    """
    ...
    # Iterate through each provider to plot their utilization line
    for provider_id, util_data in weekly_util.items():
        sorted_weeks = sorted(util_data.keys())
        util_values = [util_data[w] for w in sorted_weeks]
        
        # Plot data points with lines and markers
        ax.plot(sorted_weeks, util_values, label=provider_id, marker='o', markersize=4, linestyle='-')
        
    # Formatting
    ax.set_title(f"Weekly Utilization Over Time - {method_name.upper()} Strategy", fontweight="bold")
    ax.set_xlabel("Date (Weeks)")
    ax.set_ylabel("Utilization (%)")
    
    ax.legend(title="Healthcare Providers")
    ax.grid(True, linestyle='--', alpha=0.6)
    
    # Slant the x-axis dates so they don't overlap
    plt.setp(ax.get_xticklabels(), rotation=45, ha='right')
    fig.tight_layout()
    
    # Save the graph as an image (you can also uncomment plt.show() below to launch an interactive window)
    plt.savefig(f"utilization_{method_name}.png")
    # plt.show()
...
if __name__ == "__main__":
    ...
    try:
        ...
        for method in methods_to_run:
            ...
            plot_utilization(method, result)
